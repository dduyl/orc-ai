...
| 022 | Quota Handling Strategy | Accepted | Partial | Escalation ladder: classify quota vs rate-limit at the catch seam (`AgentCallError.kind`), rate_limit → bounded backoff, quota → downgrade model variant (ADR-021 seam) → checkpoint + pause + daemon auto-resume (ADR-010 ext.) → defer `optional` steps; always surface. Rollout: `ADR-022-ROLLOUT.md`. Implemented so far: classification, backoff, `quota_exhausted` failure reason, and full quota surfacing (Tracker `steps_json.quota` + `paused` status, ProgressEvent/step_finish `quota` payload, GUI `quota-banner`, TUI pause line, step_finish hook event, `[quota]` prefixed Tracker error) |
