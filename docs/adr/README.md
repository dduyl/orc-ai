...
| 021 | Model Routing by Task Complexity | Accepted | Implemented | Tier lookup at the call boundary (`resolveVariantTier` in `variants.ts`): complexity signal → `cheap`/`strong`; concrete model picked from the agent's advertised set against the vendored models.dev snapshot (`pickVariantModel`), filtered to configured providers; pre-emptive model applied at `session/new` (ACP `set_config_option`) / PTY flags; script steps bypass tiering |
