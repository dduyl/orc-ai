      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            steps: {
              type: "array",
              description: "Ordered list of steps (DAG: root steps have depends_on [], dependent steps list parent IDs)",
              items: {
                type: "object",
                properties: {
                  id: { type: "string" },
                  agent: { type: "string", description: "Agent name from list_prompts" },
                  depends_on: { type: "array", items: { type: "string" } },
                  task: { type: "string" },
                  context: { type: "array", items: { type: "string" } },
                },
                required: ["id", "agent", "depends_on"],
              },
            },
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            signal: { type: "boolean", description: "Quality signal: true if satisfactory, false if upstream step needs rework. Only include when the prompt asks for it." },
          },
          required: ["summary"],
        },
      },
